...
if (!class_exists('STylemixAnnouncements')) {
	class STylemixAnnouncements
	{
		...
		public function __construct()
		{
			...
			add_action('wp_ajax_stm_add_review', array($this, 'add_review'));
		...
		public function review_admin_notice() {
			$status = get_option('stm_lms_add_review_status', '');

			if (empty($status)) {
				wp_enqueue_script('stm-lms-review-notice', STM_LMS_URL . 'announcement/assets/review.js', 'jQuery', NULL, true);

				$stm_add_review = wp_create_nonce('stm_add_review');
				wp_add_inline_script( 'stm-lms-review-notice', "var stm_add_review = '{$stm_add_review}';" );

				require_once STM_LMS_PATH . '/announcement/review.php';
			}
		}

		public function add_review() {
			check_ajax_referer('stm_add_review', 'security');

			update_option('stm_lms_add_review_status', sanitize_text_field( $_GET['add_review_status'] ));
		}
